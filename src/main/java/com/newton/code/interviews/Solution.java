...
class Result {
  ...
  private static List<Integer> checkIfExists(String column, String val, UserInfo[] userInfo) {
    ...
    for (UserInfo info : userInfo) {
      switch (column) {
        case "id":
          if (info.getId().toString().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "name":
          ...
          break;
        case "username":
          if (info.getUsername().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "email":
          if (info.getEmail().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "address.street":
          if (info.getAddress().getStreet().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "address.suite":
          if (info.getAddress().getSuite().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "address.city":
          if (info.getAddress().getCity().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "address.zipcode":
          if (info.getAddress().getZipcode().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "address.geo.lat":
          if (info.getAddress().getGeo().getLat().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "address.geo.lng":
          if (info.getAddress().getGeo().getLng().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "website":
          if (info.getWebsite().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "company.name":
          if (info.getCompany().getName().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        case "company.basename":
          if (info.getCompany().getBasename().equals(val)) {
            cnt.add(info.getId());
          }
          break;
        default:
  ...
  public class UserInfo {
    ...
    public String toString() {
      return "UserInfo{" + "id=" + id + ", name='" + name + '\'' + ", userName='" + username + '\'' + ", email='" +
          email + '\'' + ", address=" + address + ", website='" + website + '\'' + ", company=" + company + '}';
    }
  }
